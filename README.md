...
- **Several photos at once.** Both sources are multi-shot: the library picker is multi-select, and *Camera* opens a burst camera (`PhotoCaptureModal`) that stays open across shots — shutter, filmstrip, ↺ undo, a 12-per-session cap — so four sides of a box are one trip rather than four. Shots stay local until *Done*, so a cancelled session uploads nothing; the uploads then run in parallel and are settled individually, so one failed file doesn't discard the other five.
